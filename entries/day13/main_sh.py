if __name__ == "__main__":
    ...
    with open('input.txt', 'r') as f:
        ...
        instructs = ((lambda l, r: (l[-1] == 'y', int(r)))(*instruct.split('=')) for instruct in instructs_raw)

        for symb_bool, magn in instructs:
        ...
        grid = [[' ' for _i in range(SIZE)] for _j in range(SIZE)]
        for r, c in points:
            grid[r][c] = '#'

        for r in grid:
            line_res = ''.join(r).strip()
            if line_res:
                print(line_res)
            else:
                break